...
CSV_DIR = Path(__file__).parent.parent.parent / "mnt" / "project"
...
def _load_mana_tasas() -> dict:
    ...
    # Formato: Sacerdote X HORA (unidades);NIVEL  — pero en realidad columnas invertidas
    tasas = {}
    for path in [
...
def get_cc_tasas():
    global _CC_TASAS
    if _CC_TASAS is None:
        _CC_TASAS = _load_cc_tasas()
    return _CC_TASAS

def get_material_tasas():
    global _MATERIAL_TASAS
    if _MATERIAL_TASAS is None:
        _MATERIAL_TASAS = _load_material_tasas()
    return _MATERIAL_TASAS

def get_mana_tasas():
    global _MANA_TASAS
    if _MANA_TASAS is None:
        _MANA_TASAS = _load_mana_tasas()
    return _MANA_TASAS
...
def aplicar_produccion(city: dict, guardar: bool = True) -> dict:
    ...
    # Aplicar producción a cada recurso
    for recurso in ["MADERA", "PIEDRA", "HIERRO", "ORO", "CARBON"]:
        ganado = tasas[recurso] * segundos
        city[recurso] = float(city.get(recurso, 0) or 0) + ganado

    # Maná
    city["MANA"] = float(city.get("MANA", 0) or 0) + tasas["MANA"] * segundos
