"""
fix_art_canvas2.py
Eternal Warriors v3.0 — Reescritura artística edificios Canvas 2D (ancla corregida)

Corre desde: E:\\0000ew V2Claude\\
Comando:     python fix_art_canvas2.py
"""
...
function lighten(c, a) {
  ...
  return `rgb(${Math.min(255,Math.round(+m[0]+(255-+m[0])*a))},${Math.min(255,Math.round(+m[1]+(255-+m[1])*a))},${Math.min(255,Math.round(+m[2]+(255-+m[2])*a))})`;
}
function darken(c, a) {
  const m=c.match(/\\d+/g); if(!m) return c;
  return `rgb(${Math.round(+m[0]*(1-a))},${Math.round(+m[1]*(1-a))},${Math.round(+m[2]*(1-a))})`;
}
...
function isoArt(ctx, x, y, w, h, cTop, cLeft, cRight, opts={}) {
  ...
  if (opts.stone!==false) {
    ctx.save(); ctx.strokeStyle='rgba(0,0,0,0.16)'; ctx.lineWidth=0.6;
...
function isoWin(ctx, x, y, w, h, color, alpha) {
  ...
  g.addColorStop(0,color.replace('rgb(','rgba(').replace(')',`,${alpha+0.15})`));
...
function drawBeam(ctx, x, y, h, color) {
  ...
  g.addColorStop(0,color.replace('rgb(','rgba(').replace(')',',0.28)')); g.addColorStop(0.5,color.replace('rgb(','rgba(').replace(')',',0.07)')); g.addColorStop(1,color.replace('rgb(','rgba(').replace(')',',0)'));
...
function artCityHall(ctx,x,y,lvl){
  ...
  for(let i=0;i<5;i++){
    isoWin(ctx,x-W*0.12,y-H*(0.22+i*0.13),W*0.14,H*0.1,'rgb(100,180,255)',wA-i*0.05);
    isoWin(ctx,x+W*0.08,y-H*(0.22+i*0.13),W*0.14,H*0.1,'rgb(100,180,255)',wA-i*0.06);
  }
  for(let i=0;i<3;i++){
    isoWin(ctx,x-W*0.66,y-H*(0.18+i*0.14),W*0.13,H*0.09,'rgb(80,150,230)',0.32-i*0.05);
    isoWin(ctx,x+W*0.44,y-H*(0.18+i*0.14),W*0.13,H*0.09,'rgb(80,150,230)',0.3-i*0.05);
  }
  ...
  ctx.beginPath(); ctx.moveTo(x-W*0.55,y-H*0.83); ctx.lineTo(x-W*0.55,y-H*0.99); ctx.stroke();
  const fw=Math.sin(tick*0.06)*4*sc;
  ctx.fillStyle='rgb(40,60,120)';
  ctx.beginPath(); ctx.moveTo(x-W*0.55,y-H*0.99); ctx.lineTo(x-W*0.55+16*sc+fw,y-H*0.95); ctx.lineTo(x-W*0.55+14*sc+fw*0.7,y-H*0.92); ctx.lineTo(x-W*0.55,y-H*0.92); ctx.closePath(); ctx.fill();
}
...
function artSanctuary(ctx,x,y,lvl){
  ...
  for(let i=0;i<6;i++){
    ...
    drawOrb(ctx,x+Math.cos(a)*W*0.42,y-H*0.94+Math.sin(a)*W*0.16,4*sc,'rgb(180,60,255)');
  }
...
function artUniversity(ctx,x,y,lvl){
  ...
  for(let i=0;i<3;i++){
    ...
    ctx.fillStyle='rgba(0,0,0,0.8)'; ctx.beginPath(); ctx.arc(wx,y-H*0.48,W*0.08,Math.PI,0); ctx.rect(wx-W*0.08,y-H*0.48,W*0.16,H*0.2); ctx.fill();
    ctx.fillStyle=`rgba(255,185,70,${wA-i*0.04})`; ctx.beginPath(); ctx.arc(wx,y-H*0.48,W*0.055,Math.PI,0); ctx.rect(wx-W*0.055,y-H*0.48,W*0.11,H*0.15); ctx.fill();
  }
}
...
function artHideout(ctx,x,y,lvl){
  ...
  ctx.fillStyle='rgb(45,25,8)'; ctx.fillRect(x-W*0.16,y-H*0.67,W*0.08,H*0.05); ctx.fillRect(x+W*0.08,y-H*0.67,W*0.08,H*0.05);
