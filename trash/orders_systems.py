...
# ── Límite de ataques diarios ─────────────────────────────────────────────────

LIMITE_ATAQUES_DIARIOS = 3          # máx ataques que un jugador A puede dar a jugador B / 24h
VENTANA_ATAQUES_SEG    = 86400.0    # 24 horas en segundos

# Jugadores excluidos de la protección (no son jugadores humanos reales)
_EXCLUIDOS_LIMITE = {"ALALAIA", "ADMIN", "JIARITO", "GINAO"}

# Jugadores excluidos de aplicar el límite como atacantes
# (estos sí pueden recibir la protección, pero no se les aplica como defensores extra)
# La protección es bidireccional: si A ataca a B 3 veces, A no puede atacar a B más.


def _es_jugador_humano(jugador: str) -> bool:
    """True si el jugador es un humano real sujeto a protección de límite."""
    return jugador.upper() not in _EXCLUIDOS_LIMITE


def _verificar_limite_ataques(jugador_atk: str, jugador_def: str, sm) -> dict:
    """
    Verifica si jugador_atk puede atacar a jugador_def.
    Retorna {"ok": True} o {"ok": False, "msg": str}.

    Regla: máx LIMITE_ATAQUES_DIARIOS ataques de A hacia B en 24h.
    Solo aplica cuando el DEFENSOR es jugador humano real.
    Inactivos, dioses, cuevas y cuentas especiales no tienen protección.
    """
    if not jugador_def:
        return {"ok": True}                         # destino no es jugador → sin límite
    jugador_def = jugador_def.upper()
    jugador_atk = jugador_atk.upper()
    if not _es_jugador_humano(jugador_def):
        return {"ok": True}                         # defensor especial → sin protección
    if jugador_atk == jugador_def:
        return {"ok": True}                         # ciudad propia → siempre permitido

    ahora = time.time()
    try:
        player_def = sm.load_player(jugador_def)
        if not player_def:
            return {"ok": True}                     # jugador inexistente → no bloquear

        # Leer y limpiar timestamps vencidos
        ataques_rec = player_def.get("ataques_recibidos", {})
        ts_lista    = [t for t in ataques_rec.get(jugador_atk, [])
                       if (ahora - t) < VENTANA_ATAQUES_SEG]

        if len(ts_lista) >= LIMITE_ATAQUES_DIARIOS:
            horas_restantes = int((VENTANA_ATAQUES_SEG - (ahora - min(ts_lista))) / 3600) + 1
            return {
                "ok":  False,
                "msg": (f"{jugador_atk} ya ha atacado a {jugador_def} "
                        f"{LIMITE_ATAQUES_DIARIOS} veces en las últimas 24h. "
                        f"Podrá atacar de nuevo en ~{horas_restantes}h."),
            }
    except Exception as e:
        print(f"[limite_ataques] Error verificando límite: {e}")
        return {"ok": True}                         # si falla la lectura, no bloquear

    return {"ok": True}


def _registrar_ataque(jugador_atk: str, jugador_def: str, sm) -> None:
    """
    Registra un ataque efectivo (llegó a combate) en player_def["ataques_recibidos"].
    Limpia automáticamente timestamps >24h para no inflar el JSON.
    Solo registra si el defensor es jugador humano real.
    """
    if not jugador_def or not _es_jugador_humano(jugador_def):
        return
    jugador_atk = jugador_atk.upper()
    jugador_def = jugador_def.upper()
    if jugador_atk == jugador_def:
    ...
    def _fn(player):
        ar = player.setdefault("ataques_recibidos", {})
        ts = [t for t in ar.get(jugador_atk, []) if (ahora - t) < VENTANA_ATAQUES_SEG]
        ts.append(ahora)
        ar[jugador_atk] = ts
    ...
    try:
        ...
        print(f"[limite_ataques] Error registrando ataque: {e}")
...
def crear_orden(
    ...
    # ── Verificar límite de ataques diarios (solo ATAQUE a jugador humano) ───
    if tipo == "ATAQUE" and jugador_dest and _es_jugador_humano(jugador_dest) and sm:
        _lim = _verificar_limite_ataques(jugador, jugador_dest, sm)
        if not _lim["ok"]:
            # Devolver unidades y oro ya descontados antes de salir
            for _nombre, _cant in unidades.items():
                _cant = int(_cant or 0)
                if _cant > 0:
                    ciudad_origen[_nombre] = int(ciudad_origen.get(_nombre, 0) or 0) + _cant
            ciudad_origen["ORO"] = oro_actual
            return {"ok": False, "msg": _lim["msg"]}
...
def _resolver_ataque(orden: dict, sm) -> dict:
    ...
    # ── Chequeo de límite al llegar (race condition: dos órdenes simultáneas) ──
    if not es_inactivo and jugador_def_nombre and _es_jugador_humano(jugador_def_nombre):
        _lim = _verificar_limite_ataques(orden["jugador"], jugador_def_nombre, sm)
        if not _lim["ok"]:
            # Orden nula — tropas regresan intactas, sin combate
            orden["unidades_sobrevivientes"] = orden["unidades"].copy()
            orden["botin"]   = {}
            orden["resultado"] = {
                "victoria": False,
                "mensaje":  _lim["msg"] + " — orden anulada, tropas regresan.",
                "rondas":   0,
            }
            orden["estado"]    = "REGRESANDO"
            orden["t_retorno"] = time.time() + (orden["t_llegada"] - orden["inicio"])
            return {"orden_id": orden["id"], "tipo": "ATAQUE", "ok": False, "msg": _lim["msg"]}
    ...
    # Registrar ataque recibido (límite 3/24h por atacante)
    if not es_inactivo and not entidad_mundo:
        _registrar_ataque(orden["jugador"], jugador_def_nombre, sm)
...
def _resolver_espionaje(orden: dict, sm) -> dict:
    ...
    # Saqueo: si fue detectado, hubo combate y el atacante ganó → saquear igual que ATAQUE
    botin = {}
    if resultado["detectado"] and resultado["combate"]:
        combate_res = resultado["combate"]
        if combate_res.get("victoria_atacante"):
            from backend.systems.combat import _calcular_saqueo
            atk_grupos_vivos = [
                {"nombre": k, "cantidad": v, "carga": 0, "es_invocacion": False}
                for jugador_dict in combate_res.get("sobrevivientes_atk", {}).values()
                for k, v in jugador_dict.items() if int(v or 0) > 0
            ]
            # Reconstruir grupos con carga real desde los stats
            from backend.systems.combat import get_stats_unidad, get_stats_invocacion, _norm, INVOCACIONES
            atk_grupos_con_carga = []
            for g in atk_grupos_vivos:
                nombre = _norm(g["nombre"])
                if nombre in INVOCACIONES:
                    st = get_stats_invocacion(nombre)
                else:
                    st = get_stats_unidad(nombre, orden["nivel_tropas"])
                atk_grupos_con_carga.append({
                    "nombre":   nombre,
                    "cantidad": int(g["cantidad"]),
                    "carga":    st.get("carga", 0),
                })
            botin = _calcular_saqueo(atk_grupos_con_carga, ciudad_def or {})
            # Descontar del defensor
            if botin and ciudad_def:
                for rec, cant in botin.items():
                    from backend.data.save_manager import safe_resource_float as _srf
                    actual = _srf(ciudad_def.get(rec, 0))
                    ciudad_def[rec] = max(0.0, actual - float(cant))
        orden["unidades_sobrevivientes"] = combate_res["sobrevivientes_atk"]
    else:
        orden["unidades_sobrevivientes"] = {k: int(v or 0) for k, v in orden["unidades"].items() if int(v or 0) > 0}

    orden["botin"]     = botin
    orden["resultado"]["saqueo"] = botin
    orden["estado"]    = "REGRESANDO"
    orden["t_retorno"] = time.time() + (orden["t_llegada"] - orden["inicio"])
    ...
    if not resultado["detectado"]:
        ...
        # Espionaje detectado → hubo combate → cuenta como ataque recibido
        if not es_inactivo and jugador_def_nombre and _es_jugador_humano(jugador_def_nombre):
            _registrar_ataque(orden["jugador"], jugador_def_nombre, sm)
...
def _resolver_espionaje_entidad(orden: dict, sm, jugador_atk: dict, entidad: dict) -> dict:
    ...
    Los dioses y cuevas son entidades hostiles — siempre detectan y combaten.
    No hay espionaje silencioso contra entidades del mundo.
    """
    ...
    # Calcular sigilo (informativo — no afecta el resultado contra entidades)
    sigilo = _calc_sigilo(unidades_atk, nivel_tropas)

    # Entidades hostiles: SIEMPRE detectan y combaten — no hay espionaje exitoso
    nombre_ent = entidad.get("NOMBRE", entidad.get("ID", "?"))
    ...
    sobrev_raw = combate["sobrevivientes_atk"].get(orden["jugador"], {})
    xp         = combate["xp_por_jugador_atk"].get(orden["jugador"], 0)

    # Reconvertir claves normalizadas (EON SUPREMO) al formato original de orden["unidades"]
    # para que _ejecutar_retorno las encuentre en la ciudad correctamente
    from backend.systems.combat import _norm as _cnorm
    norm_a_orig = {_cnorm(k): k for k in orden["unidades"]}
    sobrev = {norm_a_orig.get(_cnorm(k), k): v for k, v in sobrev_raw.items()}
...
_REPO_ALDEANO  = 15_000_000_000_000_000_000_000   # 1.5e+22
...
def _reponer_vitaminizadas(jugador: str, city: dict, sm) -> None:
    ...
    # Reponer aldeanos con el valor correcto vitaminizado
    city["ALDEANO"] = _REPO_ALDEANO
