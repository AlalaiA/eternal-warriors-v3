...
def _load_tiempo_unidades() -> dict:
    """Tiempo base de entrenamiento en minutos por unidad básica."""
    result = {}
    path = CSV_DIR / "tiempo_base_produccion_unidades_basicas.csv"
    if not path.exists():
        return result
...
def _load_invocaciones() -> dict:
    """Características de invocaciones: tiempo base y costo maná."""
    result = {}
    ...
    with open(path, encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            try:
                tiempo_min = row[7].strip()
                costo_mana = row[8].strip()
                nivel_min_sac = row[6].strip()
                if tiempo_min.upper() in ("NO", ""):
                    continue
                result[nombre] = {
                    "tiempo_min": float(tiempo_min.replace(",", ".")),
                    "costo_mana": float(costo_mana.replace(",", "").replace("E+", "e+").replace(" ", "")),
                    "nivel_min_sacerdote": int(nivel_min_sac) if nivel_min_sac.isdigit() else 99,
                }
...
def tiempo_entrenamiento_seg(unidad: str, nivel_cuartel: int) -> float:
    """Segundos por unidad básica entrenada en un cuartel de nivel dado."""
    tiempos = get_tiempo_unidades()
    reducciones = get_reduccion_cuartel()
    base_min = tiempos.get(unidad.upper(), 300)
    red = reducciones.get(nivel_cuartel, 0) / 100
    return base_min * 60 * (1 - red)

def tiempo_invocacion_seg(invocacion: str, nivel_templo: int) -> float:
    """Segundos por unidad invocada en un templo de nivel dado."""
    invs = get_invocaciones()
    rebajas = get_rebaja_templo()
    inv = invs.get(invocacion.upper(), {})
    base_min = inv.get("tiempo_min", 9000)
    reb = rebajas.get(nivel_templo, 0) / 100
    return base_min * 60 * (1 - reb)
...
def iniciar_cola_cuartel(city: dict, cuartel_key: str, unidad: str,
    ...
    # Verificar que no hay cola activa en ese cuartel
    colas = city.get("COLAS", [])
    for cola in colas:
        if cola.get("tipo") == cuartel_key and cola.get("cantidad_hecha", 0) < cola.get("cantidad_total", 0):
            return {"ok": False, "msg": f"{cuartel_key} ya tiene una cola activa"}

    # Tiempo por unidad
    t_seg = tiempo_entrenamiento_seg(unidad, nivel_cuartel)
    if t_seg <= 0:
...
def iniciar_cola_templo(city: dict, templo_key: str, invocacion: str,
    ...
    # Verificar cola activa en ese templo
    colas = city.get("COLAS", [])
    for cola in colas:
        if cola.get("tipo") == templo_key and cola.get("cantidad_hecha", 0) < cola.get("cantidad_total", 0):
            return {"ok": False, "msg": f"{templo_key} ya tiene una cola activa"}
    ...
    # Tiempo por unidad
    t_seg = tiempo_invocacion_seg(invocacion, nivel_templo)
...
def cancelar_cola(city: dict, tipo: str) -> dict:
    """Cancela la cola activa de un cuartel o templo. Devuelve maná si era templo."""
    colas = city.get("COLAS", [])
    for i, cola in enumerate(colas):
        if cola.get("tipo") == tipo:
            # Si era templo devolver maná proporcional a lo no invocado
            ...
            colas.pop(i)
            city["COLAS"] = colas
            return {"ok": True, "msg": f"Cola {tipo} cancelada"}
    return {"ok": False, "msg": f"No hay cola activa en {tipo}"}
